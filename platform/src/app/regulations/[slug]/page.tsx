...
export async function generateMetadata({params}:{params:Promise<{slug:string}>}):Promise<Metadata>{
  ...
  const entry=allRegulations[slug as Slug];
  if(!entry)return{};
  return{
    title:entry.title,
    description:entry.description,
    keywords:entry.keywords,
    alternates:{canonical:`/regulations/${slug}`},
    robots:{index:true,follow:true},
    openGraph:{title:`${entry.title} | Emissa`,description:entry.description,url:`/regulations/${slug}`,type:'article'},
  };
}
...
export default async function RegulationPage({params}:{params:Promise<{slug:string}>}){
  ...
  const entry=allRegulations[slug as Slug];
  if(!entry)notFound();
  return <SeoExpansionPage entry={entry} hubPath="/regulations" hubLabel="Regulatory Intelligence"/>;
}
