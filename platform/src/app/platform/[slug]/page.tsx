...
export async function generateMetadata({params}:{params:Promise<{slug:string}>}):Promise<Metadata>{
  ...
  const entry=platformPages[slug as Slug];
  if(!entry)return{};
  const title=`${entry.h1} Supplier Compliance Module`;
  return{
    title,
    ...
    openGraph:{type:'website',title:`${title} | Emissa`,description:entry.description,url:`/platform/${slug}`},
...
export default async function PlatformModulePage({params}:{params:Promise<{slug:string}>}){
  ...
  return <><SeoExpansionPage entry={entry} hubPath="/platform" hubLabel="Platform"/><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(schema)}}/></>;
}
