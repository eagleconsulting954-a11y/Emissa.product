...
const allTools={...tools,...phase3Tools};
...
export function GET(){
  const paths=[
    '/resources','/blog','/for','/compare','/tools','/templates','/docs','/research','/research/methodology','/research/state-of-supplier-compliance','/research/supplier-compliance-maturity-model','/regulatory-calendar','/epr-state-tracker',
    ...
    ...Object.keys(allComparisons).map(slug=>`/compare/${slug}`),
    ...Object.keys(allTools).map(slug=>`/tools/${slug}`),
    ...Object.keys(allTemplates).map(slug=>`/templates/${slug}`),
