...
export default async function CapaPage(){
  ...
  return <AdminProductShell title="Corrective Actions" eyebrow="CAPA, remediation and closure control" active="/admin/capa" actions={<a href="#new-capa">New CAPA</a>}>
    ...
    <section className="ccPanel" id="new-capa"><div className="ccPanelHead"><div><span>Create corrective action</span><small>Real production record with audit history</small></div></div><form action={createCapa} className="ccFormGrid">
      <label><span>Organization</span><select name="organizationId" required><option value="">Select organization</option>{orgs.map(o=><option key={o.id} value={o.id}>{o.name}</option>)}</select></label>
      <label><span>Finding / issue</span><input name="title" required placeholder="Expired supplier certificate"/></label>
      <label><span>Owner</span><input name="ownerEmail" type="email" placeholder="owner@company.com"/></label>
      <label><span>Due date</span><input name="dueDate" type="date"/></label>
      <label className="wide"><span>Root cause</span><textarea name="rootCause" rows={3} placeholder="Why did the issue occur?"/></label>
      <label className="wide"><span>Corrective action</span><textarea name="correctiveAction" required rows={3} placeholder="What must be completed to close the finding?"/></label>
      <div className="wide"><button className="ccGoldButton" type="submit">Create CAPA</button></div>
    </form></section>
