...
export default async function CommunicationsPage(){
  ...
  return <AdminProductShell title="Supplier Communications" eyebrow="Request evidence, send reminders and preserve outreach history" active="/admin/communications" actions={<a href="/admin/suppliers">Supplier 360</a>}>
    ...
    <section className="commLayout"><article className="ccPanel"><div className="ccPanelHead"><div><span>Send supplier request</span><small>Messages are delivered through Resend and logged in AuditEvent</small></div></div><form action={sendSupplierMessage} className="ccFormGrid oneCol"><label><span>Supplier</span><select name="supplierId" required><option value="">Select supplier</option>{suppliers.map(s=><option key={s.id} value={s.id}>{s.name} · {s.organization.name} · {s.email}</option>)}</select></label><label><span>Subject</span><input name="subject" required defaultValue="Compliance information request"/></label><label><span>Message</span><textarea name="message" rows={9} required defaultValue={'Please provide the outstanding compliance information or supporting evidence requested by our team.\n\nIf you have already submitted the requested information, no further action is required.'}/></label><button className="ccGoldButton" type="submit">Send supplier email</button></form></article><article className="ccPanel"><div className="ccPanelHead"><div><span>Communication templates</span><small>Common supplier compliance outreach</small></div></div><div className="templateStack"><div><b>Evidence request</b><small>Request certificates, declarations or supporting documents.</small></div><div><b>Expiration reminder</b><small>Notify a supplier before compliance evidence expires.</small></div><div><b>Corrective action</b><small>Send CAPA requirements and closure expectations.</small></div><div><b>Buyer questionnaire</b><small>Request missing data required for a customer response.</small></div><div><b>CBAM / EPR data request</b><small>Collect regulatory product, packaging or emissions information.</small></div></div></article></section>
