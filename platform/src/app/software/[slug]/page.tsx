...
export default async function SoftwarePage({params}:{params:Promise<{slug:string}>}){const {slug}=await params;const entry=commercialPages[slug as Slug];if(!entry)notFound();const schema={'@context':'https://schema.org','@type':'SoftwareApplication',name:entry.h1,applicationCategory:'BusinessApplication',applicationSubCategory:'Supplier Compliance Software',operatingSystem:'Web',url:`https://emissa.tech/software/${slug}`,description:entry.description,offers:{'@type':'Offer',price:'3500',priceCurrency:'USD',category:'subscription'},provider:{'@type':'Organization',name:'Emissa',url:'https://emissa.tech/'}};return <><SeoExpansionPage entry={entry} hubPath="/software" hubLabel="Supplier Compliance Software"/><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(schema)}}/></>;}
