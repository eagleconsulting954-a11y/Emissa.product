...
export default function MarketingHome() {
  return <main className="landingExact luxuryLanding">
    ...
    <section className="leFuture"><div className="sectionIntro"><span>SUPPLIER COMPLIANCE LIBRARY</span><h2>Explore the product, requirement, system and workflow behind supplier compliance.</h2><p>Use Emissa’s public knowledge layer to move from search intent and regulatory research to an operational supplier compliance system.</p></div><div className="leFeatureGrid">{seoClusters.map(([href,title,description],index)=><article className="leFeature" key={href}><div className="featureNumber">{String(index+1).padStart(2,'0')}</div><h3><a href={href}>{title}</a></h3><p>{description}</p><a className="leSecondary" href={href}>Explore {title} <i>↗</i></a><div className="featureLine"></div></article>)}</div></section>
    ...
    <footer className="leFooter">
      ...
      <div><h4>Regulations</h4><a href="/regulations">Regulatory Library</a><a href="/regulations/eudr">EUDR</a><a href="/regulations/cbam">CBAM</a><a href="/regulations/epr-california">California EPR</a><a href="/regulations/epr-maryland">Maryland EPR</a></div>
