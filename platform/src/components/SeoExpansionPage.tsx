...
import type { RegulatoryStatus } from '@/lib/regulatoryStatus';
...
export default function SeoExpansionPage({ entry, hubPath, hubLabel, downloadHref, reviewMeta }:{ entry:SeoEntry; hubPath:string; hubLabel:string; downloadHref?:string; reviewMeta?:RegulatoryStatus }) {
  ...
  const articleSchema={'@context':'https://schema.org','@type':'Article',headline:entry.h1,description:entry.description,dateModified:'2026-08-22',author:{'@type':'Organization',name:'Emissa',url:'https://emissa.tech/'},publisher:{'@type':'Organization',name:'Emissa',url:'https://emissa.tech/'},about:entry.keywords.map(name=>({'@type':'Thing',name})),mainEntityOfPage:`https://emissa.tech${hubPath}`};
  return <main className="seoPage">
    ...
    <article>
      <header className="seoHero compact"><span className="seoKicker">{hubLabel}</span><h1>{entry.h1}</h1><p>{entry.intro}</p>{reviewMeta&&<div className="seoActions"><span className="seoSecondary">Status: {reviewMeta.status}</span><span className="seoSecondary">Last reviewed: {reviewMeta.reviewed}</span></div>}<div className="seoActions"><a className="seoPrimary" href="/demo">Book a private demo</a>{downloadHref&&<a className="seoPrimary" href={downloadHref}>Download template ↓</a>}<a className="seoSecondary" href={hubPath}>Explore {hubLabel}</a></div></header>
      {reviewMeta&&<section className="seoGrid"><article className="seoCard"><span className="seoKicker">Next milestone</span><h2>{reviewMeta.nextMilestone}</h2><p>Confirm the current requirement against the official sources on this page before making a filing, scope or legal decision.</p></article></section>}
